...
def get_money(balance: Decimal):
    ...
    if money_to_get % MIN_SUM == 0:
        ...
        if money_to_get + procent <= balance:
            """Информация о снятии добавлется в список"""
            operations_history.append(('withdrawal', money_to_get))
...
def give_money(balance: Decimal):
    ...
    if money_to_give % MIN_SUM == 0:
        """ Информация о пополнении добавляется в список"""
        operations_history.append(('deposit', money_to_give))
